# 74hc381.py
from core.component import Component
from core.pin import PinDirection


class HC381(Component):
    """74HC381 — 4-bit ALU."""

    def __init__(self):
        super().__init__("74HC381")

        # Operand inputs
        for i in range(4):
            self.add_pin(f"A{i}", PinDirection.INPUT)
            self.add_pin(f"B{i}", PinDirection.INPUT)

        # Function select (S0..S3)
        for i in range(4):
            self.add_pin(f"S{i}", PinDirection.INPUT)

        # Mode: 0 = logic, 1 = arithmetic
        self.add_pin("M", PinDirection.INPUT)

        # Carry in
        self.add_pin("CIN", PinDirection.INPUT)

        # Outputs F0..F3, carry out
        for i in range(4):
            self.add_pin(f"F{i}", PinDirection.OUTPUT)

        self.add_pin("COUT", PinDirection.OUTPUT)

    def update(self, tick):
        a = 0
        b = 0
        for i in range(4):
            if self.pins[f"A{i}"].read():
                a |= (1 << i)
            if self.pins[f"B{i}"].read():
                b |= (1 << i)

        s = 0
        for i in range(4):
            if self.pins[f"S{i}"].read():
                s |= (1 << i)

        m = 1 if self.pins["M"].read() else 0
        cin = 1 if self.pins["CIN"].read() else 0

        if m == 0:
            # Logic mode: pick a logical operation using S
            if s == 0x0:
                f = a
            elif s == 0x1:
                f = b
            elif s == 0x2:
                f = ~a & 0xF
            elif s == 0x3:
                f = ~b & 0xF
            elif s == 0x4:
                f = a & b
            elif s == 0x5:
                f = a | b
            elif s == 0x6:
                f = a ^ b
            elif s == 0x7:
                f = ~(a & b) & 0xF
            else:
                # fall back to zero for unused codes
                f = 0
            cout = 0
        else:
            # Arithmetic mode (simplified: add/sub variations)
            if s == 0x0:
                result = a + b + cin
            elif s == 0x1:
                result = a + (~b & 0xF) + cin
            elif s == 0x2:
                result = (a - b + cin) & 0x1F
            elif s == 0x3:
                result = (b - a + cin) & 0x1F
            else:
                result = a + cin

            f = result & 0xF
            cout = 1 if result > 0xF else 0

        for i in range(4):
            self.pins[f"F{i}"].write((f >> i) & 1)
        self.pins["COUT"].write(cout)